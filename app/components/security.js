...
class Security {
    ...
    protect(authorisedRoles) {
        ...
        return function (req, res, next) {
            ...
            let userToken;
            if (req.cookies) {
                userToken = req.cookies[SECURITY_COOKIE];
            }

            // Retrieve user details
            if (userToken) {
                services.getUserDetails(userToken)
                    .then(response => {
    ...
    _login(req, res) {
        ...
        const callbackUrl = FormatUrl.format(returnUrl, config.services.idam.oauth2.callback_path);
    ...
    _getTokenFromCode(req) {
        ...
        const redirectUri = FormatUrl.format(hostname, config.services.idam.oauth2.callback_path);
