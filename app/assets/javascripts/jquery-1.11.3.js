...
//
    ...
    jQuery.extend({
                      ...
                      isNumeric: function (obj) {
                          ...
                          // ("0x...") subtraction forces infinities to NaN adding 1 corrects loss
                          // of precision from parseFloat (#15100)
        ...
        (function (window) {
            ...
            var i,
                ...
                pseudos = ':(' + characterEncoding + ')(?:\\((' +
                          ...
                          // prefer arguments: 1. quoted (capture 3; capture 4 or capture 5)
                          '(\'((?:\\\\.|[^\\\\\'])*)\'|"((?:\\\\.|[^\\\\"])*)")|' +
                          // 2. simple (capture 6)
                          '((?:\\\\.|[^\\\\()[\\]]|' + attributes + ')*)|' +
...
// Optimize for push.apply( _, NodeList )
             ...
             *     itself with property name the (space-suffixed) string and (if the cache is
...
// Expose support vars for convenience
            ...
            setDocument = Sizzle.setDocument = function (node) {
                ...
                // Support: IE>8
                // If iframe document is assigned to "document" variable and if iframe has been
                // reloaded, IE will throw "permission denied" error when accessing "document"
                // variable, see jQuery #13936 IE6-8 do not support the defaultView property so
	...
	---------------------------------------------------------------------- */
                ...
                if ((support.qsa = rnative.test(doc.querySelectorAll))) {
                    ...
                    assert(function (div) {
                        ...
                        // still enabled) IE8 throws error here and will not see later tests
	...
	---------------------------------------------------------------------- */
			...
			*/
                        ...
                        if (match[1].slice(0, 3) === 'nth') {
                            ...
                            match[4] =
                                +(match[4] ? match[5] + (match[6] || 1) : 2 * (match[3] === 'even'
                                                                               || match[3]
                                                                               === 'odd'));
                                   ...
                                   if (parent) {
                                       ...
                                       if (simple) {
                                           while (dir) {
                                               ...
                                               // Reverse direction for :only-* (if we haven't yet
                                               // done so)
                                               start =
                                       ...
                                       if (forward && useCache) {
                                           ...
                                           while ((node = ++nodeIndex && node && node[dir] ||
                                                          ...
                                                          // Fallback to seeking `elem` from the
                                                          // start
                                           ...
                                           // Use previously-cached element index if available
                                                               ...
                                                               (elem[expando] || (elem[expando] = {}))[type])
                                                  && cache[0] === dirruns) {
                                           diff = cache[1];
                                           ...
                                           // Use the same loop as above to seek `elem` from the
                                           // start
                                           while ((node = ++nodeIndex && node && node[dir] ||
                                                          (diff = nodeIndex = 0) || start.pop())) {
                ...
                pseudos: {
                    ...
                    // case-insensitively. The identifier C does not have to be a valid language
                    // name." http://www.w3.org/TR/selectors/#lang-pseudo
                    ...
                    'empty': function (elem) {
                        ...
                        // entity ref: 5), but not by others (comment: 8; processing instruction:
                        // 7; etc.) nodeType < 6 works because attributes (2) do not appear as
                        // children
                        for (elem = elem.firstChild; elem; elem = elem.nextSibling) {
                            if (elem.nodeType < 6) {
                                return false;
                               ...
                               while ((elem = elem[dir])) {
                                   if (elem.nodeType === 1 || checkNonElements) {
                                       ...
                                       if ((oldCache = outerCache[dir]) &&
                                           ...
                                           // Reuse newcache so results back-propagate to previous
                                           // elements
            ...
            function matcherFromGroupMatchers (elementMatchers, setMatchers) {
                var bySet = setMatchers.length > 0,
                    ...
                    superMatcher = function (seed, context, xml, results, outermost) {
                        ...
                        // Keep `i` a string if there are no elements so `matchedCount` will be
                        // "00" below Support: IE<9, Safari Tolerate NodeList properties (IE:
                        // "length"; Safari: <number>) matching elements by id
    ...
    jQuery.fn.extend({
                         ...
                         is: function (selector) {
                             return !!winnow(
                                 ...
                                 // the returned set so $("p:first").is("p:last") won't return true
...
// The deferred used on DOM ready
    ...
    function completed () {
        ...
        if (document.addEventListener || event.type === 'load' || document.readyState
            === 'complete') {
            detach();
            jQuery.ready();
        }
...
// checks a cache object for emptiness
    ...
    function internalData (elem, name, data, pvt /* Internal Use Only */) {
        ...
        if ((!id || !cache[id] || (!pvt && !cache[id].data)) && data === undefined && typeof name
            === 'string') {
            return;
        }
    ...
    jQuery.extend({
                      ...
                      // collisions) throw uncatchable exceptions if you attempt to set expando
                      // properties
                      noData: {
                          'applet ': true,
                          'embed ': true,
        ...
        mouseHooks: {
            ...
            filter: function (event, original) {
                ...
                if (event.pageX == null && original.clientX != null) {
                    ...
                    event.pageX =
                        original.clientX + (doc && doc.scrollLeft || body && body.scrollLeft || 0)
                        - (doc && doc.clientLeft || body && body.clientLeft || 0);
                    event.pageY =
                        original.clientY + (doc && doc.scrollTop || body && body.scrollTop || 0)
    ...
    if (!support.changeBubbles) {
        ...
        jQuery.event.special.change = {
            ...
            handle: function (event) {
                ...
                if (this !== elem || event.isSimulated || event.isTrigger || (elem.type !== 'radio'
                                                                              && elem.type
                                                                              !== 'checkbox')) {
    ...
    function getAll (context, tag) {
        var elems, elem,
            ...
            found = typeof context.getElementsByTagName !== strundefined
                    ...
                    typeof context.querySelectorAll !== strundefined ? context.querySelectorAll(
                        tag || '*') :
                    undefined;
...
// Mark scripts as having already been evaluated
    ...
    jQuery.extend({
                      clone: function (elem, dataAndEvents, deepDataAndEvents) {
                          ...
                          if (support.html5Clone || jQuery.isXMLDoc(elem) || !rnoshimcache.test(
                              '<' + elem.nodeName + '>')) {
    ...
    jQuery.fn.extend({
                         ...
                         domManip: function (args, callback) {
                             ...
                             if (l) {
                                 ...
                                 if (first) {
                                     scripts =
                                         jQuery.map(getAll(fragment, 'script'), disableScript);
    ...
    function actualDisplay (name, doc) {
        var style,
            ...
            display = window.getDefaultComputedStyle && (style =
                ...
                // Use of this method is a temporary fix (more like optmization) until something
                // better comes along, since it was removed from specification and supported only
                // in FF
...
// return a css property mapped to a potentially vendor prefixed property
    ...
    function getWidthOrHeight (elem, name, extra) {
        ...
        var valueIsBorderBox = true,
            ...
            isBorderBox = support.boxSizing && jQuery.css(elem, 'boxSizing', false, styles)
                          === 'border-box';
    ...
    jQuery.extend({
                      ...
                      style: function (elem, name, value, extra) {
                          ...
                          if (value !== undefined) {
                              ...
                              // cssHooks, but it would mean to define eight (for every problematic
            ...
            set: function (elem, value, extra) {
                ...
                return setPositiveNumber(elem, value, extra ?
                                                      augmentWidthOrHeight(
                                                          ...
                                                          name,
                                                          extra,
                                                          support.boxSizing && jQuery.css(elem,
                                                                                          'boxSizing',
                                                                                          false,
                                                                                          styles)
                                                          === 'border-box',
    ...
    if (!support.opacity) {
        jQuery.cssHooks.opacity = {
            ...
            set: function (elem, value) {
                ...
                // attribute #6652 if value === "", then remove inline opacity #12685
                if ((value >= 1 || value === '') &&
    ...
    jQuery.cssHooks.marginRight = addGetHookIf(support.reliableMarginRight,
                                               function (elem, computed) {
                                                   if (computed) {
                                                       ...
                                                       // returns wrong value for margin-right Work
                                                       // around by temporarily setting element
            ...
            '*': [function (prop, value) {
                ...
                if (start && start[3] !== unit) {
                    ...
                    do {
                        ...
                        // Use a string for doubling factor so we don't accidentally see scale as
                        // unchanged below
                        scale = scale || '.5';
                        ...
                        // And breaking the loop if scale is unchanged or perfect, or if we've just
                        // had enough
        ...
        for (prop in props) {
            ...
            if (rfxtypes.exec(value)) {
                ...
                if (value === (hidden ? 'hide' : 'show')) {
                    ...
                    // If there is dataShow left over from a stopped hide or show and we are going
                    // to proceed with show, we should pretend to be hidden
                              ...
                              set: function (elem, value) {
                                  ...
                                  while (i--) {
                                      ...
                                      if (jQuery.inArray(jQuery.valHooks.option.get(option), values)
                                          ...
                                          // Support: IE6
                                          // When new option element is added to select box we need
                                          // to force reflow of newly added node in order to
                                          // workaround delay of initialization properties
                                          try {
...
// Radios and checkboxes getter/setter
    ...
    jQuery.extend({
                      ...
                      attrHooks: {
                          type: {
                              set: function (elem, value) {
                                  if (!support.radioValue && value === 'radio' && jQuery.nodeName(
                                      elem, 'input')) {
                                      // Setting the type on a radio button after the value resets
                                      // the value in IE6-9 Reset value to default in case type is
                                      // set after value during creation
                                      var val = elem.value;
                         ...
                         toggleClass: function (value, stateVal) {
                             ...
                             return this.each(function () {
                                 if (type === 'string') {
                                     ...
                                     // above saved it). Otherwise bring back whatever was
                                     // previously saved (if anything), falling back to the empty
    ...
    jQuery.extend({
		...
		*/
                      ...
                      ajax: function (url, options) {
                          ...
                          s.url =
                              ((url || s.url || ajaxLocation) + '').replace(rhash, '')
                          ...
                          if (!s.hasContent) {
                              ...
                              if (s.data) {
                                  cacheURL =
                                      (s.url += (rquery.test(cacheURL) ? '&' : '?') + s.data);
                              ...
                              if (s.cache === false) {
                                  s.url = rts.test(cacheURL) ?
                                      ...
                                      // Otherwise add one to the end
                                          cacheURL + (rquery.test(cacheURL) ? '&' : '?') + '_='
                                          + nonce++;
                              }
                          }
...
// document keepScripts (optional): If true, will include scripts passed in the html string
        ...
        if (self.length > 0) {
            jQuery.ajax({
                ...
                self.html(selector ?
                              ...
                              // div Exclude scripts to avoid IE 'Permission Denied' errors
