...
class FormatName {
    ...
    static formatMultipleNamesAndAddress (persons, content, applicantAddress) {
        if (persons) {
            ...
            const formattedNames = Object.keys(persons)
                .map(
                    key => FormatName.getNameAndAddress(persons[key], content.of, applicantAddress))
                .join(separator);
    ...
    static delimitNames (formattedNames, separator, contentAnd) {
        ...
        if (lastCommaPos > -1) {
            return `${formattedNames.substring(0,
                lastCommaPos)} ${contentAnd} ${formattedNames.substring(
                lastCommaPos + separator.length)}`;
        }
